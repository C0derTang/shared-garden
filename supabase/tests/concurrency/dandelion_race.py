#!/usr/bin/env python3
"""Actual overlapping Dandelion sessions in an empty disposable local Supabase DB.
...
parser.add_argument("container", nargs="?", default="supabase_db_shared-garden-dandelion32")
...
def command(app="dandelion_race_control"):
...
def overlap(first, second, error=None, first_member=1, second_member=2, hold=2):
    a = session("dandelion_race_a", "begin;\n" + signed(first_member) + first + f";\nselect pg_sleep({hold});\ncommit;\n")
    b = None
    try:
        wait_for("dandelion_race_a", "wait_event='PgSleep'")
        b = session("dandelion_race_b", "begin;\n" + signed(second_member) + second + ";\ncommit;\n")
        wait_for("dandelion_race_b", "wait_event_type='Lock'")
