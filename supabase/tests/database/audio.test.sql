...
select has_function('public','attest_audio_upload',array['uuid','uuid','integer','integer','integer','text'],'trusted validation attestation');


select ok(not has_table_privilege('authenticated','private.media_uploads','SELECT'),'registry is not directly readable');
select ok(not has_function_privilege('authenticated','public.attest_audio_upload(uuid,uuid,integer,integer,integer,text)','EXECUTE'),'browser cannot attest');
...
select ok(has_function_privilege('service_role','public.attest_audio_upload(uuid,uuid,integer,integer,integer,text)','EXECUTE'),'trusted server can attest');
...
insert into public.flower_unlocks(type_key) values('bluebell');
select public.plant_flower('bluebell');
create temporary table media_test_values(key text primary key,value jsonb);
grant all on media_test_values to authenticated;
create function pg_temp.flower() returns uuid language sql as $$ select id from public.flowers where type_key='bluebell' limit 1 $$;
...
declare v jsonb;
begin
 v:=public.create_media_upload(gen_random_uuid(),pg_temp.flower(),'audio/webm',100,replacement);
...
begin
 ...
 insert into storage.objects(bucket_id,name,metadata) values('garden-media',v->>'final_path','{"size":100,"mimetype":"audio/wav"}');
 perform public.attest_audio_upload((v->>'id')::uuid,(v->>'lease_id')::uuid,100,28,1,repeat('a',64));
end $$;
set local role authenticated;
select throws_ok($$select public.create_media_upload(gen_random_uuid(),(select id from public.flowers where type_key='cactus'),'audio/webm',100)$$,'22023','media_wrong_flower','wrong flower rejected');
select throws_ok($$select public.create_media_upload(gen_random_uuid(),pg_temp.flower(),'image/png',100)$$,'22023','media_wrong_flower','photo cannot attach to an audio flower');
select throws_ok($$select public.create_media_upload(gen_random_uuid(),pg_temp.flower(),'audio/webm',12582913)$$,'22023','media_invalid_request','byte limit enforced');
...
select throws_ok($$select public.attest_audio_upload(pg_temp.media('original'),gen_random_uuid(),100,28,1,repeat('a',64))$$,'42501',null,'member cannot invoke attestation');
...
select throws_ok($$insert into storage.objects(bucket_id,name) values('garden-media',pg_temp.media('original')::text||'/audio.wav')$$,'42501',null,'member cannot insert final object');
...
select throws_ok($$select public.create_media_upload(gen_random_uuid(),pg_temp.flower(),'audio/webm',100)$$,'42501',null,'unapproved authenticated user cannot create');
...
select throws_ok($$select public.create_media_upload(gen_random_uuid(),pg_temp.flower(),'audio/webm',100,(select (value->>'id')::bigint from media_test_values where key='entry'))$$,'42501','media_wrong_owner','partner cannot request author replacement');
select pg_temp.actor(1);
...
set local role authenticated;
select throws_ok($$select public.edit_flower_entry((select (value->>'id')::bigint from media_test_values where key='entry'),jsonb_build_object('media_id',pg_temp.media('late')))$$,'22023','The edit window has ended','late audio replacement rejected');
...
select ok(not exists(select 1 from cleanup_result,jsonb_array_elements(value) item where item->>'final_path' in (pg_temp.media('original')::text||'/audio.wav',pg_temp.media('replacement')::text||'/audio.wav')),'cleanup never selects submitted final objects');
...
set local role authenticated;
select throws_ok($$select public.edit_flower_entry((select (value->>'id')::bigint from media_test_values where key='entry'),jsonb_build_object('media_id',pg_temp.media('purpose')))$$,'22023','The edit window has ended','audio cannot change expired edit permission');
reset role;
...
reset role;
insert into public.flowers(type_key,spot,planted_by,planted_at,planted_day) values('bluebell',99,1,clock_timestamp(),(select garden_day from private.garden_clock_at(clock_timestamp())));
set local role authenticated;
...
select is(public.create_media_upload((select request_id from private.media_uploads where id=pg_temp.media('purpose')),pg_temp.flower(),'audio/webm',100),public.media_upload_state(pg_temp.media('purpose')),'identical create retry returns original intent');
select throws_ok($$select public.create_media_upload((select request_id from private.media_uploads where id=pg_temp.media('purpose')),pg_temp.flower(),'audio/webm',101)$$,'22023','media_idempotency_conflict','changed request-key parameters rejected');
select throws_ok($$select public.attest_audio_upload(pg_temp.media('purpose'),gen_random_uuid(),100,28,1,repeat('a',64))$$,'22023','media_expired_or_changed','duplicate or stale attestation cannot replace trusted metadata');
...
set local role authenticated;
select lives_ok($$select public.media_read_path(pg_temp.media('replacement'))$$,'historical finalized audio is still authorized');
